...
## Побудова рекурсивного спуск-парсера для граматики if-else
...
# (T) = термінал, (F) = нетермінал (як на фото)
...
❯ python3 main.py
ПЕРЕВІРКА: Рядок 1 — простий if без else
Вхідний рядок:
if ( expr ){
    stmt1;
}
...
Результат: ПРАВИЛА ГРАМАТИКИ ДОТРИМАНІ

ПЕРЕВІРКА: Рядок 2 — if з else та кількома операторами
Вхідний рядок:
if ( expr ){
    stmt;
}
...
ПЕРЕВІРКА: Рядок 3 — вкладені if-else if-else
Вхідний рядок:
if ( expr ){
    stmt;
    stmt;
